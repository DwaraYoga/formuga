...
import { uangKeluar } from "@/src/db/schema";
...
export async function GET() {
    try {
        ...
        const data = await db.select().from(uangKeluar).orderBy(desc(uangKeluar.createdAt));
...
export async function POST(req: Request) {
    try {
        ...
        const result = await db.insert(uangKeluar).values({
        jmlUang: Number(body.jmlUang), // Pastikan diconvert ke Number

...
export default function UangMasukPage() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const res = await fetch('/api/admin/uang-masuk');
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const res = await fetch(`/api/admin/uang-masuk/${id}`, { method: 'DELETE' });
            if (res.ok) {
                fetchData();
            }
        } catch (err) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const url = isEditing 
                ? `/api/admin/uang-masuk/${isEditing}` 
                : '/api/admin/uang-masuk';
    ...
    return (
        ...
        {/* Header */}
        <div>
            <h1 className="text-4xl font-black text-slate-900">UANG MASUK</h1>
